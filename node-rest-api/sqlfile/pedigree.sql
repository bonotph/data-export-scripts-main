/* To export the medical info and pedigree of participants
 *
 * Note:
 * Update the timestamp and PC if necessary
 * 
 */
...
-- get the hpo info
LEFT JOIN (
	SELECT DISTINCT inner_h.patient_id FROM patient_hpo_view_view AS inner_h WHERE json_length(inner_h.hpo) > 0
) AS h ON p.id = h.patient_id

-- get the cancer info
LEFT JOIN (
	SELECT DISTINCT inner_c.patient_id FROM cancer_detail_view AS inner_c WHERE inner_c.isDeleted = 0
) AS c ON p.id = c.patient_id

-- get the pedigree info
LEFT JOIN (
	SELECT DISTINCT inner_f.patient_id
    FROM file_view AS inner_f 
    WHERE 
